...
##        self.updateResultSignal.connect(self.updateResult)
    ...
    def updateOutput(self): #将结果信息显示在界面
        ...
        while p_ss_conn.poll(): #查询是否接收到状态信息
            a = p_ss_conn.recv()
            ss.append(a)
        if ss:
##            self.resultplainTextEdit.appendPlainText('\n'.join(ss))
            for i in ss: #相应状态实时显示
                if i.startswith("INFO:[stats]scrapy.spider.domainspider.downloader/request_count:"):
                    self.requestcountLabel.setText(i[64:].strip()) #改变请求页面数
                elif i.startswith("INFO:[stats]scrapy.spider.domainspider.downloader/response_count:"):
                    self.responsecountLabel.setText(i[65:].strip()) #改变响应页面数
                elif i.startswith("INFO:[stats]scrapy.spider.domainspider.downloader/response_bytes:"):
                    self.responsebytesLabel.setText(i[65:].strip()) #改变响应字节数
                elif i.startswith("INFO:[stats]scrapy.spider.domainspider.downloader/response_status_count/200:"):
                    self.response200countLabel.setText(i[76:].strip()) #改变成功响应页面数(200)
                elif i.startswith("INFO:[stats]scrapy.spider.domainspider.item_scraped_count:"):
                    self.itemscrapedLabel.setText(i[58:].strip()) #改变抓取条目数
                elif i.startswith("INFO:[stats] downloaditem :"):
                    self.itemdownloadLabel.setText(i[27:].strip()) #改变成功下载条目数
        ...
        if p_c_conn.poll(): #查询是否接收到控制信息
            ...
            if c == "stoped crawl":
                ...
                self.statusLabel.setText(u"结束运行") #改变运行状态Label
...
##            f.close()
        ...
        self.spiderProcess = Process(target = spiderProcess_entry, args=(c_s_conn, c_c_conn, c_r_conn, c_ss_conn)) #实例化spider进程,指向进程入口的函数
        self.spiderProcess.start()

        self.tabWidget.setCurrentIndex(1) #界面转到显示Tab
...
def spiderProcess_entry(start_conn, contrl_conn, result_conn, stats_conn): #spider进程入口
    rule = start_conn.recv()
    start_conn.send("start crawl")

    the_spider = setupspider(rule, contrl_conn, result_conn, stats_conn) #实例化setupspider类
    the_spider.run()
...
if __name__ == "__main__":
    ...
    p_ss_conn, c_ss_conn = Pipe() #stats connection
